...
TOL = {
    ...
    torch.bfloat16: {"atol": 0.5, "rtol": 0.05},
...
def test_adaln_non_power_of_two_hidden():
    ...
    fused_in = _adaln_inputs(b, t, n, dtype, dev)
    ref_in = {k: v.detach().clone().requires_grad_(True) for k, v in fused_in.items()}
    _h_f, out_f = _run_adaln(lambda **kw: fused_adaln_residual_rmsnorm(**kw), fused_in, t)
    _h_r, out_r = _run_adaln(lambda **kw: adaln_residual_rmsnorm_ref(**kw), ref_in, t)
    _close(out_f, out_r, dtype, "out")
