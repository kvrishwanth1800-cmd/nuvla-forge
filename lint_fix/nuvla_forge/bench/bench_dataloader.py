...
def measure_loader(loader, n_batches, device, prefetch=False):
    ...
    *compute*; measured here, with no compute to hide behind, it can only
    ...
    return {"samples_per_sec": seen / dt, "batches": n_batches, "seconds": dt}
...
def measure_idle(loader, device, n_batches=30, prefetch=False):
    ...
    return {"wall_ms": wall_ms, "gpu_busy_ms": busy_ms,
...
def main():
    ...
    configs = [
        ("naive (2 workers, no pin)", build_naive_loader, {"num_workers": 2}, False),
        ("tuned (8 workers, pinned)", build_fast_loader, {"num_workers": 8, "prefetch_factor": 4}, False),
        ("tuned + cuda prefetch",     build_fast_loader, {"num_workers": 8, "prefetch_factor": 4}, True),
        ("tuned + 16 workers",        build_fast_loader, {"num_workers": 16, "prefetch_factor": 6}, True),
    ]
    ...
    for name, build, over, prefetch in configs:
        ...
        try:
            ...
            # loader-construction failure should show up as this config's
            # result row, not crash the whole sweep.
            row = {"config": name, "error": str(exc)}
